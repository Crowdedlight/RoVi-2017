...
vector<cv::Point2f> Vision::getPointsFromMaker1(cv::Mat &image) {
    ...
    //get resulting mask
    applyHSVSegmentation(image, segmented, mask, cv::Scalar(118,255,255), cv::Scalar(137,255,255), cv::MORPH_CLOSE, cv::Size(25,25));

    vector<cv::KeyPoint> keypoints;
    cv::Mat imageKeypoints; //9630 - 12658
    applyBlobDetecting(mask, keypoints, imageKeypoints, 0, 81, 2500, 20000, 0, 0.8, 0);
...
vector<cv::Point2f> Vision::getPointsFromMaker1(cv::Mat &image, cv::Mat &imgOut) {
    cv::Mat segmented, mask;
    //get resulting mask
    applyHSVSegmentation(image, segmented, mask, cv::Scalar(118,255,255), cv::Scalar(137,255,255), cv::MORPH_CLOSE, cv::Size(10,10));
    ...
    for(const auto& p : trackedPoints)
        cv::circle(imageKeypoints, p, 3, cv::Scalar(0,0,255), -1,8,0);
